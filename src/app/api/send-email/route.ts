...
export async function POST(request: NextRequest) {
  try {
    ...
    const { to, resetUrl, userName = "there", type } = body;

    if (!to || !resetUrl || !type) {
      return NextResponse.json(
        { error: "Missing required fields: to, resetUrl, type" },
        { status: 400 },
      );
    }
    ...
    if (type === "password-reset") {
      ...
      subject = "Verify Your Email - SDFM 2520";
      emailContent = {
        html: generateEmailVerificationHTML({ resetUrl, userName, to }),
        text: generateEmailVerificationText({ resetUrl, userName, to }),
      };
    } else {
      return NextResponse.json(
    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <p>Thank you for signing up for SDFM 2520! To complete your registration, please verify your email address by clicking the button below:</p>

          <div style="text-align: center;">
            <a href="${resetUrl}" class="button">Verify My Email</a>
          </div>
...
function generateEmailVerificationText({
  ...
  return `
Verify Your Email - SDFM 2520

Hi ${userName},

Thank you for signing up for SDFM 2520! To complete your registration, please verify your email address by clicking or copying the following link into your browser:
